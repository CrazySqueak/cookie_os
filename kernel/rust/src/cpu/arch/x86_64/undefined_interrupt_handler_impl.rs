...
pub(super) fn init_undefined(idt: &mut InterruptDescriptorTable) {
    init_ud_body!(@youvegottabefuckingkiddingme, idt, undefined_handler,
        // deep inhale
        // N.B. we don't set default (undefined vector) handlers for exceptions
        // as they may have error codes (which the default handler doesn't support) and
        // are so important that they need a specialised handler
